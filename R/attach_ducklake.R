...
#' @param ducklake_path file name (and directory if not in working directory) of ducklake
#' @param ducklake_alias short alias to use instead of ducklake_path when referring to ducklake database
#' @param create_if_not_exists create a new ducklake if it does not already exist (default is FALSE).
#' If FALSE then attaches in READ_ONLY mode (read_only will be separate option in future)
#'
...
#' @param parquet_data_path directory where the parquet files are / should be stored (default is "data_files")
#' @param override_data_path if data path is different to default should this override the default (default is FALSE)
#'
...
#'    ducklake_path = tempfile(pattern = "metadata.ducklake"),
#'    ducklake_alias = "my_ducklake",
#'    create_if_not_exists = TRUE,
#'    ducklake_encrypt = FALSE,
#'    parquet_data_path = tempdir())
#'
...
attach_ducklake <- function(conduckdb,
                            ...
                            ducklake_encrypt = TRUE,
                            parquet_encrypt = TRUE,
                            parquet_data_path = "data_files",
                            override_data_path = FALSE) {

  check_duckdb_connection(conduckdb)

  stopifnot("ducklake_path should be a character path to a ducklake" = is.character(ducklake_path))
  ...
  if (ducklake_type == "duckdb") {
    dp <- DBI::dbQuoteLiteral(conduckdb, paste0("ducklake:", ducklake_path))
  }
  ...
  if (ducklake_type == "sqlite") {
    dp <- DBI::dbQuoteLiteral(conduckdb, paste0("ducklake:sqlite:", ducklake_path))
  }

  pe <- glue::glue_sql("ENCRYPTED {parquet_encrypt}", .con = conduckdb)

  pdp <- glue::glue_sql("DATA_PATH {parquet_data_path}", .con = conduckdb)

  odp <- glue::glue_sql("OVERRIDE_DATA_PATH {override_data_path}", .con = conduckdb)

  # build for attach
  if(!create_if_not_exists & ducklake_encrypt) {
    attach_sql <- glue::glue_sql("ATTACH {dp} AS {da} ({cine}, {de}, READ_ONLY);", .con = conduckdb)
  } else if (!create_if_not_exists & !ducklake_encrypt) {
    attach_sql <- glue::glue_sql("ATTACH {dp} AS {da} ({cine}, READ_ONLY);", .con = conduckdb)
  } else if (create_if_not_exists &  ducklake_encrypt) {
    attach_sql <- glue::glue_sql("ATTACH {dp} AS {da} ({cine}, {pe}, {pdp}, {odp}, {de});", .con = conduckdb)
  } else {
    attach_sql <- glue::glue_sql("ATTACH {dp} AS {da} ({cine}, {pe}, {pdp}, {odp});", .con = conduckdb)
  }
